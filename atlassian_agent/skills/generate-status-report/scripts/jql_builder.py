...
def build_project_query(
    ...
    if days_back:
        if not isinstance(days_back, int) or days_back < 0:
            raise ValueError(
                f"days_back must be a non-negative integer, got: {days_back}"
            )
...
def build_blocked_query(project_key: str, high_priority_only: bool = False) -> str:
...
def build_completed_query(project_key: str, days_back: int = 7) -> str:
...
def build_risk_query(project_key: str, include_overdue: bool = True) -> str:
...
def build_unassigned_query(project_key: str, exclude_done: bool = True) -> str:
