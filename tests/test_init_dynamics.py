...
def _get_pkg_name():
    ...
    Prefer the actual package directory (the sole top-level dir holding an
    ``__init__.py`` that isn't ``tests``) so the test is correct regardless of
    the checkout/worktree directory name; fall back to the directory name.
    """
    import pathlib
