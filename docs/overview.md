# atlassian-agent — Concept Overview
...
```json
{
  "mcpServers": {
    "atlassian-agent": {
      "command": "uv",
      "args": ["run", "--with", "atlassian-agent", "atlassian-mcp"],
      "env": {}
    }
  }
}
...
```bash
atlassian-mcp --transport streamable-http --port 8001
```
